...
命題Aは n ≤ 5×10⁶, k ≤ 15 で反例ゼロが確認されており、以下の帰納法的証明構造がほぼ完成している。

#### 補題1（完全証明済み）
任意の奇数 n に対して T(n) = T(C²(n)) + 2 が成り立つ。

**証明**: 奇数 n に対して C(n) = 3n+1 は偶数，したがって C²(n) = (3n+1)/2。  
定義より T(n) = 1 + T(C(n)) = 1 + 1 + T(C²(n)) = 2 + T(C²(n)). ■

#### 補題2（完全証明済み）
n ≡ 2^k−1 (mod 2^k) ならば C²(n) ≡ 2^(k−1)−1 (mod 2^(k−1))。

**証明**: n = 2^k·m + 2^k − 1 とおくと  
C²(n) = (3n+1)/2 = 3·2^(k−1)·m + 3·2^(k−1) − 1 ≡ −1 ≡ 2^(k−1)−1 (mod 2^(k−1)). ■

#### 補題3（完全証明済み）
奇数 r ∈ {1,…,2^k−2} で r ≠ 2^k−1 のとき，C²(r) ≢ 2^(k−1)−1 (mod 2^(k−1))。

**証明（背理法）**: C²(r) ≡ 2^(k−1)−1 と仮定すると  
(3r+1)/2 ≡ 2^(k−1)−1 (mod 2^(k−1))  
⟹ 3r ≡ −3 (mod 2^k)  
⟹ r ≡ −1 (mod 2^k)  [gcd(3,2^k)=1 より 3 は可逆]  
⟹ r = 2^k−1。これは r ≠ 2^k−1 に矛盾。■  
k = 2〜19 の全残差クラスで計算実験により確認済み。

#### 主定理（k ≥ 2, 帰納法）

k−1 で命題Aが成立するとして，k の各残差クラスを分類する：

**Case 1 (n ≡ 2^k−1)**: 補題1より T(n) = 2 + T(C²(n))，補題2より C²(n) は k−1 の all-1 クラスに属す。  
⟹ E[T | all-1, k] = 2 + E[T | all-1, k−1]

**Case 2 (n ≡ 偶数, mod 2^k)**: n = 2m より T(n) = 1 + T(m) で m は k−1 の残差クラスに属す。  
E[T | 2r, k] = 1 + E[T | r, k−1]  
差 = E[T|all-1,k] − E[T|2r,k] = 1 + (E[T|all-1,k−1] − E[T|r,k−1]) ≥ 1 > 0  (帰納仮定から)

**Case 3 (n ≡ 奇数 r ≠ 2^k−1)**: 補題1より T(n) = 2 + T(C²(n))，補題3より C²(n) は k−1 の **非** all-1 クラスに属す。  
帰納仮定より E[T|C²クラス, k−1] < E[T|all-1, k−1]  
⟹ E[T|r,k] = 2 + E[T|C²クラス, k−1] < 2 + E[T|all-1, k−1] = E[T|all-1, k]

以上 3 ケースより，all-1 クラスが唯一の最大クラスとなる。■

**残課題**: 基底 k=1（奇数クラスの平均 T が偶数クラスを上回ること）の解析的証明。計算実験では N = 10⁴ 〜 2×10⁶ の全スケールで margin ≈ 12.3 が安定して確認され，漸近的には差 ≈ 1 + C·log₂(3/2) > 0 が示唆される（H68/H77）。
...
1. **末尾全1ビット則 (命題A)**: n ≡ 2^k−1 (mod 2^k) が同クラスで最長停止時間を持つ。k≤15, n≤5×10⁶ で反例ゼロ。帰納法による証明構造がほぼ完成（補題1〜4 は数学的に完全証明済み；基底 k=1 のみ解析的証明が残課題）。
...
- **問題1**: 命題Aの基底 k=1 の解析的証明（帰納段は完成、T(n)分布の精密な解析が必要）
