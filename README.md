...
n ≤ 2×10⁸ の大規模探索・58仮説の検証・情報理論的解釈を行いました。  
...
- **命題A（末尾全1ビット則）** — n ≡ 2^k−1 (mod 2^k) の剰余クラスは同クラス内で最大停止時間を持つ。k≤12, n≤10⁷ で反例ゼロ（未証明・計算実験による観察）
...
> **注記:** 本研究は計算実験に基づく探索的研究であり、証明を含みません。  
> 命題Aを含む観察は未証明です。Δ公式の定式化は既存文献への徹底的な調査を経ておらず、独自性の確認には査読が必要です。
