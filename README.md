...
> 58の仮説と2億件の計算実験で、コラッツ予想の構造に迫る。

コラッツ予想 (3n+1問題) を計算実験的に探索したPythonプロジェクトです。  
n ≤ 2×10⁸ の大規模探索・58仮説の検証・情報理論的解釈・新遅延記録の発見を行いました。  
VPS (1vCPU / 960MB RAM) 上で約8時間の計算実験です。
...
- **命題A（末尾全1ビット則）** — n ≡ 2^k−1 (mod 2^k) の剰余クラスは同クラス内で最大停止時間を持つ。k≤12, n≤10⁷ で反例ゼロ
- **Δ公式の理論的導出** — kビット全1クラスの停止時間差分 Δ = 6.309 ± 0.080 を理論式 `Δ = 2 + C(N)×log₂(3/2)` から導出。最良近似定数は 4log₂3 ≈ 6.340
- **新遅延記録** — n = 169,941,673 (953ステップ) を 2×10⁸ 範囲の新チャンピオンとして同定
- **情報理論的収束根拠** — 全軌道を通じた情報損失 −0.1340 bits/step が、全軌道の1への収束を情報理論的に示唆
